...
class LookupEmbedding(nn.Module):
    def __init__(self, st, langs):
        ...
        for param_idx in self.used_param_idxs:
            param_values_dict = self.walsinfo.param_idx_to_param_value_idx_to_param_value_name[
                param_idx]
            ...
            logger.info(
                "Parameter {} (offset +{}):".format(param_idx, num_values))
            logger.info(" >> Name: {}".format(param_name))
            logger.info(" >> ID: {}".format(param_id))
            logger.info(
                " >> Parameter value space size: {}".format(param_space_size))

            for (param_value_idx, param_value_name) in param_values_dict.items():
                logger.info("     >> Parameter value {} (offset +{}), name: {}".format(
                    param_value_idx, param_value_idx-1, param_value_name))
        ...
        logger.info("Maximum language ID token vocab idx: {}".format(
            max_lang_vocab_idx))

        lang_map = [0] * (max_lang_vocab_idx+1)

        for k, lang in enumerate(langs):
            logger.info("Lang ID {} ({}) maps to {}".format(
                lang[1], lang[0], k))
            lang_map[lang[1]] = k
        self.lang_map = torch.tensor(lang_map, dtype=torch.long).cuda()
        memusage(self.lang_map, "lang_map")

        self.max_lang_vocab_idx = torch.tensor(
            [max_lang_vocab_idx], dtype=torch.long).cuda()
        ...
        self.gold_mask = torch.zeros(
            (num_embeddings, embedding_dim), dtype=torch.bool).cuda()
        memusage(self.gold_mask, "gold_mask")
        # self.denom_mask = torch.zeros((embedding_dim, embedding_dim), dtype=torch.bool).cuda()

        self.softmax_sizes = []
        #   Bad coding style but works great
        for param_idx in self.used_param_idxs:
            param_value_space_size = len(
                self.walsinfo.param_idx_to_param_value_idx_to_param_value_name[param_idx])
        ...
        for k, lang in enumerate(langs):
            ...
            for param_idx in self.used_param_idxs:
                ...
                if param_idx in param_dict:
                    ...
                    self.gold_mask[k][offset: offset +
                                      param_value_space_size] = True
    ...
    def forward(self, x):
        ...
        y = torch.index_select(
            self.lang_map, 0, xcap.flatten()).reshape(x.shape)
...
class WordOrLanguageEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        singleton_masks = x.unsqueeze(0) == self.lang_id_idxs.reshape(
            (-1,) + (1,) * len(x.shape))
